...
function req(path, method = 'GET', data = null) { 
    return new Promise((resolve, reject) => { 
        const body = data ? JSON.stringify(data) : null; const r = http.request({ hostname: 'localhost', port: process.env.PORT || 8080, path, method, headers: { 'Content-Type': 'application/json' } }, 
            res => { 
